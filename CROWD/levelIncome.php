        ...
        <div class="block-header">
            <div class="row">
                <div class="col-lg-5 col-md-8 col-sm-12">                        
                    <h2><a href="javascript:void(0);" onclick="window.history.back();" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a> Level Income</h2>
                    <ul class="breadcrumb">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover dataTable js-exportable table-custom mb-0">
                                ...
                                <?php
                                    ...
                                    $count=0;
                                    $queryBonus=mysqli_query($con,"SELECT a.levelIncome,a.level,a.dateTime,b.user_id,b.name,c.user_id AS childId,c.name AS childName FROM meddolic_user_level_income a, meddolic_user_details b, meddolic_user_details c WHERE a.memberId=b.member_id AND a.childId=c.member_id AND CAST(a.dateTime AS date) BETWEEN '$cal_date' AND '$cal_date1' ".$query." ORDER BY a.dateTime DESC");
                                    while($valBonus=mysqli_fetch_assoc($queryBonus)){
                                        $count++; ?>
                                    <tr>
                                        <td><?= $count?></td>
                                        <td><?= $valBonus['user_id']?></td>
                                        <td><?= $valBonus['name']?></td>
                                        <td>$  <?= $valBonus['levelIncome']?></td>
                                        <td><i class="fa fa-clock-o"></i> <?= date("d-m-Y H:i:s", strtotime($valBonus['dateTime']));?></td>
                                        <td>Level <?= $valBonus['level']?></td>
                                        <td><?= $valBonus['childName']?></td>
                                        <td><?= $valBonus['childId']?></td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
