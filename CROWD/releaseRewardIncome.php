...
<?php  
  ...
  $queryReward=mysqli_query($con,"SELECT summaryId,memberId,rewardId,rewardIncome,dueDate FROM meddolic_user_reward_income_summary WHERE rewardStatus=1   ORDER BY summaryId ASC");
while($valNo=mysqli_fetch_assoc($queryReward)){
    ...
    $dueDate=$valNo['dueDate'];
     ...
     $queryNew=mysqli_query($con,"INSERT INTO meddolic_user_reward_income_details (summaryId,memberId,rewardId,rewardIncome,releaseDate) VALUES ('$summaryId','$memberId','$rewardId','$rewardIncome','$d')");
...
} echo "<script>alert('Reward Income Released  Successfully!!!');window.top.location.href='rewardRelease';</script>"; ?>
